...
public class Repulo extends Thread{
     ...
     * @param ido
     */
    public Repulo(int kepX, int kepY, int kepSzelesseg, int kepMagassag, Image kep, boolean eltalaltak, int hatterSzelesseg, int hatterMagassag, SzalVezerlo szalVezerlo, long ido) {
        this.kepX = kepX;
        this.kepY = kepY;
    ...
    @Override
    public void run() {
        while (kepX < hatterSzelesseg) {
            mozdul();
            frissit();
            alszik();
        }
        if (kepX >= hatterSzelesseg) {
            szalVezerlo.repuloTorol(this);
        }
    }
